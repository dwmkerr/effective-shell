...
- **Being able to read from standard input** - this allows us to pipe inputs from _other_ tools into our programs (see [Thinking in Pipelines](../../01-core-skills/07-thinking-in-pipelines/index.mdx) for more on this), we also want to read and process line-by-line, in case the input is very large
...
The weird looking characters such as `/033[92m` are ANSI control codes to set the colour of the output - you can read all about them in [Useful Patterns for Shell Scripts](../../03-shell-scripting/23-useful-patterns-for-shell-scripts/index.mdx) in the section 'Colouring Output'.
...
If you need a reminder how to use the `chmod` tool and `ln` tool to install scripts, check the chapter [Shell Script Essentials](../../03-shell-scripting/18-shell-script-essentials/index.mdx) under the section 'Installing Your Script'.
