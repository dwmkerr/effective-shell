...
import AnnotatedCommand from '../../../src/components/AnnotatedCommmand/AnnotatedCommand.tsx';
...
We can use one of these operators with a motion to alter a range of text. Let's see some of the basic operators:
...
Let's look at some other ways to combine motions and operators to manipulate text.
...
At this stage we have seen just a few of the essential Vim features. Let's update our cheatsheet.
...
Let's add these changes to Git - and show one more way to use Vim. Enter the line of text below in the shell but don't press 'Enter' to execute it yet!

```
$ git commit -m 'added more detail on motions'
```

This command will commit the changes - but we haven't added any yet! Rather than fixing this by moving around the command line, let's just open the current shell command line in Vim. Press `^x^e` - this is the shell command to open the current line in the editor:
...
| <AnnotatedCommand annotation="Write and quit">:wq</AnnotatedCommand> | (Vim closes and `git commit` compltes) |
...
- show an asciinema of what vim can do ;) - show how to turn the plain text into markdown, recording can show lowercase text in the headings being headerised with tilde and visual mode, insert mode to add the title symbol
- is a title with Vim in it better or will it turn people off?
- visual mode - headingise with tilde
- Caret as a component like this `<Caret style='select' />` that uses CSS post to format the next letter, with insert mode caret
- Edit command - go back to apostropme in `-m 'message'` and `ci'`
- add reading list to this and next chapter
- essential next steps: line numbers, style, configuration
- 'selection' component
- style guide showing components in action, with the online editor would be v cool, as an appendix
- two of the examples that have braces in the annotation are showing extra braces
