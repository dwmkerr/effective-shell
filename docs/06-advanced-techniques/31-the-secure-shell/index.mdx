...
The details of Asymmetric encryption are a complex subject, the low-level details are beyond the scope of this book, we'll cover the only essential concepts and techniques in this chapter[^1].
...
- We use a non-standard port, 8022, rather than the default port which is 22

Let's use `scp` to copy our `lookup` program to our remote machine:

```
$ scp ~/effective-shell/samples/programs/lookup/lookup.py effective-shell-aws-linux:~
