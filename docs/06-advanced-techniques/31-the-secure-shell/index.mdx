...
There are many other options available for the SSH config file, you can see them all with `man ssh_config`. We'll see some other options in [Chapter 33 - Master the Multiplexer](../33-master-the-multiplexer/index.mdx).
...
In [Chapter 33 - Master the Multiplexer](../33-master-the-multiplexer/index.mdx) we'll look at some great ways to improve upon this, for now if you forget the Enter/Tidle/Dot escape sequence the easiest thing to do is to close your terminal program.
...
Let's see this in action by copying the `lookup` tool we created in [Chapter 30 - How to Avoid Scripting](../30-how-to-avoid-scripting/index.mdx) to our server. Then we'll run it, save some definitions, and copy them _back_ to our local machine.
