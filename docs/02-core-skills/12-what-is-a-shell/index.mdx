...
- Quickly enter commands, see the history of commands and quickly restructure commands (see [Chapter 8 - Fly on the Command Line](../../02-core-skills/08-fly-on-the-command-line/index.mdx))
- Navigate through the file system, moving from folder to folder (see [Chapter 1- Navigating Your System](../../01-transitioning-to-the-shell/02-navigating-your-system/index.mdx)), which makes it easier for an operator to navigate the file system
- Chain the output of commands together - for example, taking the output of one basic program, such as the `tree` program we saw, and writing it to a file (see [Chapter 7 - Thinking in Pipelines](../../02-core-skills/07-thinking-in-pipelines/index.mdx))
