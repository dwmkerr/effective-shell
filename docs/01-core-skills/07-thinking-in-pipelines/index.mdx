...
If you are not familiar with `ls` (the _list directory contents_ command) then check [Chapter 2 - Navigating Your System](/appendices/shell-basics/). The first part of the output isn't too important - but we can see we have three files in the special `/dev/` (short for _device_ folder). We can also see the associated file descriptors.
...
We saw in [Chapter 4 - Becoming a Clipboard Gymnast](/appendices/shell-basics/) that we could **pipe** the output of this command into the `sort` command to order it and then into the `uniq` command to remove duplicates, like this:
...
In [Chapter 4 - Becoming a Clipboard Gymnast](/appendices/shell-basics/) we saw a trick to remove formatting from text in the clipboard. Here's a similar trick to reverse the contents of the clipboard:
...
Piping into your pager is a really useful trick - you can read more about pagers in [Chapter 5 - Getting Help](/appendices/shell-basics/).
...
[^1]: Technically there is another layer here, which is the `tty`. You can see this by running `tty` in the shell. We'll talk more about this in the [Interlude - What is a Shell](../../01-core-skills/12-what-is-a-shell/index.mdx) section.
[^2]: Check [Chapter 4 - Becoming a Clipboard Gymnast](/appendices/shell-basics/) for how to do this on a Linux or Windows machine.
